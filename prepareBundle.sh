...
function syncResources() {
    ...
    if [ -d "$widgetFolder/build/static" ]; then
        ...
        $CPCMD --include='*.js' --include='*/' --exclude='*' "$widgetFolder/build/static" bundle/resources 2>/dev/null
        #rsync -a --include='*.js' --include='*/' --exclude='*' "$widgetFolder/build/static" bundle/resources 2>/dev/null
    else
